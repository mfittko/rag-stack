...
- `POST /query` — Embeds the query text, performs similarity search in Postgres using pgvector, and supports adaptive/manual `minScore` plus optional filters. (`graphExpand` query-time expansion has been removed; graph traversal is transitioning to Apache AGE on Postgres: https://age.apache.org/)
